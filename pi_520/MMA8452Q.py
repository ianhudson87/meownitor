...
# https://www.controleverything.com/content/Accepylorometer?sku=MMA8452Q_I2CS#tabs-0-product_tabset-2
...
class MMA8452Q():
    ...
    def mode_configuration(self):
        ...
        MODE_CONFIG = (MMA8452Q_ODR_800 | MMA8452Q_MODE_NORMAL |
                       MMA8452Q_MODE_ACTIVE)
        bus.write_byte_data(MMA8452Q_DEFAULT_ADDRESS,
                            MMA8452Q_REG_CTRL_REG1, MODE_CONFIG)
    ...
    def data_configuration(self):
        ...
        DATA_CONFIG = (MMA8452Q_DATA_CFG_FS_4)
        bus.write_byte_data(MMA8452Q_DEFAULT_ADDRESS,
                            MMA8452Q_REG_XYZ_DATA_CFG, DATA_CONFIG)

    def read_accl(self):
        """Read data back from MMA8452Q_REG_STATUS(0x00), 7 bytes
        Status register, X-Axis MSB, X-Axis LSB, Y-Axis MSB, Y-Axis LSB, Z-Axis MSB, Z-Axis LSB"""
        data = bus.read_i2c_block_data(
            MMA8452Q_DEFAULT_ADDRESS, MMA8452Q_REG_STATUS, 7)
        # Convert the data
        xAccl = (data[1] * 256 + data[2]) / 16
        if xAccl > 2047:
            xAccl -= 4096
